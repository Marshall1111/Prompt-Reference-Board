...
function createKindergartenBookTextOverlay({ page, childName, width, height }) {
  ...
  <g transform="rotate(-2 ${padding * 1.35} ${panelY - 18})"><rect x="${padding}" y="${panelY - Math.round(safeHeight * 0.045)}" width="${Math.round(safeWidth * 0.32)}" height="${Math.round(safeHeight * 0.06)}" rx="10" fill="#f2bd65" opacity="0.96"/><text x="${padding + 18}" y="${panelY - Math.round(safeHeight * 0.008)}" fill="#5b3d28" font-family="Arial, sans-serif" font-size="${Math.round(safeWidth * 0.019)}" font-weight="700" letter-spacing="1">${escapeKindergartenOverlayText(tag)}</text></g>
