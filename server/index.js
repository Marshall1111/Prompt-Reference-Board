...
const BASE_BOOK_THEME_DEFINITIONS = [
  ...
  { id: "career", name: "职业认知书", englishName: "My First Jobs", title: "我的第一本职业认知书", parts: [
    ["doctor", "医生", "Doctor"], ["teacher", "老师", "Teacher"], ["firefighter", "消防员", "Firefighter"], ["chef", "厨师", "Chef"], ["police", "警察", "Police Officer"], ["engineer", "工程师", "Engineer"], ["scientist", "科学家", "Scientist"], ["artist", "艺术家", "Artist"], ["dentist", "牙医", "Dentist"], ["gardener", "园丁", "Gardener"], ["writer", "作家", "Writer"], ["singer", "歌手", "Singer"],
    ["astronaut", "宇航员", "Astronaut"], ["pilot", "飞行员", "Pilot"], ["carpenter", "木匠", "Carpenter"], ["fisherman", "渔夫", "Fisherman"], ["athlete", "运动员", "Athlete"], ["soldier", "军人", "Soldier"], ["magician", "魔术师", "Magician"],
...
app.get("/api/body-book/shares/:token", async (req, res) => {
  try {
    ...
    res.json(toPublicSharedBodyBook(current));
  } catch (error) {
    res.status(error.status || 500).json({ message: error.publicMessage || "分享内容暂时不可访问。" });
  }
});
...
app.get("/api/body-book/projects", requireWebAccount, async (req, res) => {
  try {
    ...
    const projects = (await Promise.all(ownedSessions.map(synchronizeBodyBookSession)))
      ...
      .sort((left, right) => String(getBodyBookContentUpdatedAt(right)).localeCompare(String(getBodyBookContentUpdatedAt(left))));
...
app.post("/api/body-book/projects/:sessionId/share", requireWebAccount, async (req, res) => {
  try {
    assertRegisteredAccount(req);
    const session = await readBodyBookSession(req.params.sessionId);
    if (!session) throw createHttpError(404, "这本认知书工程不存在或已删除。");
    assertWebAccountOwnsBodyBookSession(req, session);
    const current = await synchronizeBodyBookSession(session);
    if (!current.pages.some((page) => page.status === "succeeded" && page.result?.imageUrl)) {
    ...
    res.status(error.status || 500).json({ message: error.publicMessage || "创建分享链接失败，请稍后重试。" });
  }
});

app.delete("/api/body-book/projects/:sessionId/share", requireWebAccount, async (req, res) => {
  try {
    assertRegisteredAccount(req);
    const session = await readBodyBookSession(req.params.sessionId);
    if (!session) throw createHttpError(404, "这本认知书工程不存在或已删除。");
    assertWebAccountOwnsBodyBookSession(req, session);
    const next = await withBodyBookSessionSyncLock(session.sessionId, async () => {
      const current = await readBodyBookSession(session.sessionId);
      if (!current) throw createHttpError(404, "这本认知书工程不存在或已删除。");
      assertWebAccountOwnsBodyBookSession(req, current);
      return saveBodyBookSession({
        ...current,
    ...
    res.json({ share: toPublicBodyBookShare(next.share) });
  } catch (error) {
    res.status(error.status || 500).json({ message: error.publicMessage || "关闭分享失败，请稍后重试。" });
  }
});
...
function buildCartoonBodyBookCoverPrompt(theme, profile) {
  ...
  return `Use the uploaded child photos only as loose visual-trait reference. Keep the child's broad age impression, hairstyle silhouette, hair color, general skin-tone family, and one or two recognizable clothing details, but do NOT reproduce the exact face, photo pose, photographic skin texture, or a realistic likeness. Create one square 1:1 bilingual cover for the 0-3 year-old ${baseTheme.name}. Draw one single child in a premium family-friendly animated-feature 3D cartoon style: rounded sculpted forms, expressive simplified facial features, soft matte materials, gentle global illumination, and polished non-photorealistic digital animation art. Use a pure solid white (#FFFFFF) full-bleed background: no warm yellow, cream, beige, ivory, off-white, paper texture, grain, gradient, shadow wash, or colored backdrop. Theme scene: ${getCartoonCoverScene(baseTheme)}. Every visible subject, including the child, vehicles, animals, objects, icons, and decorations, must use this same animated-feature 3D cartoon style; do not mix in photographs, realistic product renders, realistic animals, or cutout photos. The main title should read exactly: "${baseTheme.englishName}". The Chinese subtitle should read exactly: "${baseTheme.title}". Beneath it, add the small English line: "A Bilingual Book for Babies" and the small Chinese line: "中英双语 · 0-3岁宝宝启蒙". Match the original photography-version cover typography treatment: rounded, highly legible sans-serif type; make the English title playful with a refined natural rainbow palette, while keeping all Chinese text dark and clear; add a small pink circular badge in the upper-right that reads exactly "0-3岁适用". Do not write "Cartoon Edition", "动画卡通版", "手绘卡通版", or any other style label anywhere in the image. Keep the child, every word, arrow, prop, and important decoration entirely inside the central 80% by 80% safe area, at least 10% away from every canvas edge. No extra people, logos, watermark, border, crowded room, deep perspective, photorealistic skin pores, photography, live-action lighting, photographic collage, or unreadable decorative text.`;
}
...
function buildCartoonBodyBookPartPrompt(part, order, theme) {
  ...
  const profile = getBodyBookPromptProfile(theme);
  const concept = getBodyBookLearningConcept(part);
  return `Use the uploaded child photos only as loose visual-trait reference. Retain broad age impression, hairstyle silhouette, hair color, general skin-tone family, and one or two recognizable clothing details, but do NOT reproduce the exact face, photo pose, photographic skin texture, or a realistic likeness. Create one square 1:1 bilingual ${baseTheme.name} learning card for ages 0-3. The sole learning concept is "${concept.english} / ${concept.chinese}". Render the heading exactly: "${concept.chinese} ${concept.english}" and the supporting sentence exactly: "${part.copy}". Draw exactly one child in a premium family-friendly animated-feature 3D cartoon style: rounded sculpted forms, expressive simplified facial features, soft matte materials, gentle global illumination, and polished non-photorealistic digital animation art. Use a pure solid white (#FFFFFF) full-bleed background: no warm yellow, cream, beige, ivory, off-white, paper texture, grain, gradient, shadow wash, or colored backdrop. Page scene: ${getCartoonPageScene(baseTheme, concept, part)}. Every visible subject, including the child, vehicle, animal, prop, arrow, icon, and background decoration, must use this same animated-feature 3D cartoon style. Never mix in photography, realistic animals, realistic vehicles, product renders, or photographic cutouts. Theme-appropriate clothing and props may take priority when needed, while retaining the child's hairstyle, skin-tone family, and a small recognizable clothing detail. Make the learning concept immediate and unmistakable, with at most one clear arrow or visual cue and one or two matching ${profile.icons}. Keep the child, every word, arrow, prop, and important decoration entirely inside the central 80% by 80% safe area, at least 10% away from every canvas edge. The only allowed text is the exact heading and supporting sentence specified above. The page's sequence/order is internal metadata only: never render a page number, page count, numeral, badge, or any other extra text. Never use photorealism, realistic skin pores, photography, live-action lighting, photographic collage, extra people, a busy room, deep background, watermark, border, unrelated objects, internal production labels, or unreadable decorative text.`;
}
...
function getCartoonCoverScene(theme) {
  ...
  const scenes = {
    ...
    transport: "the child surrounded by six clearly separated friendly, recognizable animated cartoon vehicles; never real vehicles or photographs",
    animal: "the child surrounded by six friendly, recognizable animated cartoon animals; never real animals or photographs",
...
function getCartoonPageScene(theme, concept, part) {
  ...
  const scenes = {
    ...
    career: `the child wearing a child-safe animated cartoon version of the ${concept.english} profession outfit and using one simple matching cartoon prop`,
    ...
    transport: `the child safely playing with one clearly recognizable animated cartoon toy ${concept.english}; never show a real, photographed, ride-on, or rendered vehicle`,
    animal: `the child beside one friendly, recognizable animated cartoon ${concept.english} animal; never show a real, photographed, or rendered animal`,
...
function buildKindergartenBookPagePrompt(part, order, theme = getBookTheme("kindergarten"), personalization = {}) {
  ...
  const outfitContinuity = part?.key === "wake-up"
    ? "This is the only pajama page: dress the child in one cute, comfortable, hand-drawn pajama set. Do not use the reference photo's clothing colors, pattern, or outfit details for this page."
    : "The FIRST reference photo is the mandatory clothing-trait source for every non-wake-up page. Preserve its main garment colors, pattern or graphic feeling, neckline, sleeve length, garment silhouette, and shoe style; redraw this same outfit in the hand-drawn picture-book style on every page. Do not substitute a generic school outfit or change its colors, pattern, clothing style, or shoes. The backpack appears only when the story action calls for it and is otherwise put away.";
  return `Use the uploaded child photos only as a loose visual-trait reference: retain the child's broad age impression, hairstyle silhouette, hair color, and general skin-tone family, but do NOT reproduce the exact face, photo pose, photographic skin texture, or a realistic portrait. Redraw the child as the same simple, slightly abstract, fully hand-drawn picture-book character on every page, with colored-pencil outlines, watercolor fills, uneven crayon texture, and a soft expressive rounded face. Create one square 1:1 INNER STORY PAGE about "${concept.english} / ${concept.chinese}". Story moment: ${story}. ${profile.cardScene}. ${outfitContinuity} Mandatory page-specific scene direction: ${visualDirection} Use a warm cream full-bleed paper page, hand-torn paper shapes, and a few restrained ${profile.icons}. Reserve a clear 15% crop-safe margin on all four sides: keep every important part of the child, all text, and all essential decorative elements inside this safe area. Keep the child unmistakably central; any teacher, caregiver, or classmates must be simplified supporting characters, never additional focal children. Render this text clearly in the illustration: Chinese heading exactly "${concept.chinese}"; small English heading exactly "${concept.english}"; Chinese story sentence exactly "${story}"; small English story sentence exactly "${englishStory}". Put the two headings in a calm upper-corner hand-drawn label and the story sentences in a clean lower-third caption area, always within the crop-safe margin. The page order is internal metadata only: never render a page number, page count, numeral, sequence label, badge, or any other extra text. Never use photorealism, camera lighting, realistic skin pores, 3D animation, busy rooms, deep perspective, clutter, distressing emotions, or unrelated props.`;
}
...
function getBodyBookPartVisualDirection(themeId, partKey) {
  const directions = {
    ...
    kindergarten: {
      "first-day": "Show a confident standing pose at home holding a small paper sun, with a morning window and one simple calendar sticker.",
      "wake-up": "Show the child sitting up in a small cozy bed and stretching both arms toward a painted morning sun, with no realistic bedroom clutter.",
      "wash-and-dress": "Show the child happily patting their face with a small towel beside one illustrated toothbrush cup, with no realistic bathroom.",
      backpack: "Show the child proudly putting on a small backpack beside one hand-drawn star sticker.",
      arrive: "Show the child arriving at a friendly, simple hand-painted kindergarten gate with flowers and a sun; never show a real school logo or readable sign.",
      goodbye: "Show a gentle hug goodbye with one simplified, warmly drawn adult caregiver shown from the side, then the child giving a small wave. Keep the child calm and supported, never crying or distressed.",
      teacher: "Show one friendly hand-drawn teacher kneeling at the child's height with an open welcoming gesture and a small star sticker, keeping the child as the visual focus.",
      cubby: "Show the child carefully placing the small backpack into one simple cubby with a picture-symbol label only; never include readable words or a school name.",
      friends: "Show the child and two simplified hand-drawn classmates smiling and waving hello in a roomy, uncluttered play area.",
      play: "Show the child floor-seated with two simplified classmates, happily stacking a small tower of colorful blocks.",
      snack: "Show the child seated at a simple little table enjoying a small fruit snack, with one cup and no branded packaging.",
      lunch: "Show clean hands beside a simple illustrated sink, then a small lunch tray with vegetables and rice in the same gentle scene.",
      outdoor: "Show the child running or jumping along a simple chalk path outdoors with one ball and a few friendly classmates in the distance.",
      nap: "Show the child peacefully lying on a small mat under a light blanket, hugging one tiny cloud pillow, with calm moon and star stickers.",
      "miss-home": "Show a mild thoughtful expression while a friendly hand-drawn teacher sits nearby at child height, offering a comforting hug and a small heart-shaped cushion. The child must look safe and gently reassured, never upset or crying.",
      pickup: "Show the child happily walking toward one simplified caregiver at a kindergarten gate during warm afternoon light, with a painted setting sun and a small tomorrow-star sticker."
    }
  };
...
async function createBodyBookImageJob(session, slot, provider, providers, referenceMetadata = session.references || [session.reference]) {
  ...
  const identityRule = isKindergartenBook
    ...
    : isCartoonBook
      ? "All uploaded reference photos are of the same child. Use them only for broad visual traits: approximate age impression, hairstyle silhouette, hair color, general skin-tone family, and one or two clothing cues. Do not reproduce the exact face, pose, photo background, photographic skin texture, or a realistic likeness. Draw one simplified animated-feature 3D cartoon child, never a portrait photograph, and do not blend the references into multiple children."
...
function normalizeBodyBookSession(session) {
  ...
  return {
    ...
    createdAt: session?.createdAt || null,
    updatedAt: session?.updatedAt || null,
    lastContentUpdatedAt: session?.lastContentUpdatedAt || null,
    completedAt: session?.completedAt || null,
    savedAt: session?.savedAt || null,
    share: normalizeBodyBookShare(session?.share),
    reference: references[0] || reference,
    references,
    chargedJobIds: Array.isArray(session?.chargedJobIds) ? session.chargedJobIds.map(String).filter(Boolean) : [],
...
function toPublicBodyBookSession(session, accountId = "") {
  ...
  return {
    ...
    createdAt: current.createdAt,
    updatedAt: current.updatedAt,
    lastContentUpdatedAt: getBodyBookContentUpdatedAt(current),
    completedAt: current.completedAt,
    savedAt: current.savedAt,
...
function normalizeBodyBookShare(share) {
  const token = String(share?.token || "").trim().toLowerCase();
  const enabled = Boolean(share?.enabled) && isSafeBodyBookShareToken(token);
  return {
    token: enabled ? token : "",
    enabled,
...
function toPublicBodyBookShare(share) {
  return {
    enabled: Boolean(share?.enabled),
    createdAt: share?.enabled ? share.createdAt || null : null
  };
}

function isSafeBodyBookShareToken(value) {
  return /^[a-f0-9]{32}$/i.test(String(value || ""));
}

function createBodyBookShareToken() {
...
async function findSharedBodyBookSession(token) {
  ...
  const session = sessions.find((item) => {
    const share = normalizeBodyBookShare(item?.share);
    return share.enabled && share.token === safeToken;
  });
...
function getBodyBookContentUpdatedAt(session) {
  const current = normalizeBodyBookSession(session);
  // `updatedAt` is a technical persistence timestamp: it also changes while
  // job statuses are synchronized in the background.  The library must only
  // show a time caused by creating, editing, or regenerating book content.
  return current.lastContentUpdatedAt || current.savedAt || current.createdAt || null;
}
...
function toPublicBodyBookLibraryItem(session) {
  ...
  return {
    ...
    sessionId: current.sessionId,
    savedAt: current.savedAt || null,
    updatedAt: getBodyBookContentUpdatedAt(current),
    theme: toPublicBookTheme(theme, current.layoutVersion),
    title: theme.name,
...
function createKindergartenBookTextOverlay({ page, childName, width, height }) {
  ...
  // Keep all server-rendered story text inside the same 15% crop-safe area
  // required from the image model, so print trimming cannot cut the wording.
  const padding = Math.round(safeWidth * 0.15);
  const panelHeight = Math.round(safeHeight * (isCover ? 0.22 : 0.19));
  const panelY = safeHeight - panelHeight - Math.round(safeHeight * 0.15);
  ...
  <rect x="${padding}" y="${panelY}" width="${safeWidth - padding * 2}" height="${panelHeight}" rx="${Math.round(safeWidth * 0.025)}" fill="#fffaf0" fill-opacity="0.93" stroke="#e2b787" stroke-width="${Math.max(2, Math.round(safeWidth * 0.003))}"/>
