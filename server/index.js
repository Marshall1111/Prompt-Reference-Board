...
function buildKindergartenBookPagePrompt(part, order, theme = getBookTheme("kindergarten"), personalization = {}) {
  ...
  const outfitContinuity = part?.key === "wake-up"
    ? "This is the only pajama page: turn the clothing traits from the FIRST reference photo into a matching soft pajama version, preserving its main colors and pattern feeling but using a sleepwear silhouette."
    : "Outfit continuity is mandatory: use the FIRST reference photo as the exact clothing-trait source for every non-wake-up page. Preserve its main garment colors, pattern or graphic feeling, neckline, sleeve length, garment silhouette, and shoe style; redraw this same outfit in the hand-drawn picture-book style on every page. Do not substitute a generic school outfit or change its colors, pattern, clothing style, or shoes. The backpack appears only when the story action calls for it and is otherwise put away.";
  return `Use the uploaded child photos only as a loose visual-trait reference: retain the child's broad age impression, hairstyle silhouette, hair color, and general skin-tone family, but do NOT reproduce the exact face, photo pose, photographic skin texture, or a realistic portrait. The FIRST reference photo is also the outfit reference. Redraw the child as the same simple, slightly abstract, fully hand-drawn picture-book character on every page, with colored-pencil outlines, watercolor fills, uneven crayon texture, and a soft expressive rounded face. Create one square 1:1 INNER STORY PAGE, page ${order}, about "${concept.english} / ${concept.chinese}". Story moment: ${story}. ${profile.cardScene}. ${outfitContinuity} Mandatory page-specific scene direction: ${visualDirection} The outfit-continuity rule above overrides every clothing, accessory, or shoe instruction in the scene direction. Use a warm cream full-bleed paper page, hand-torn paper shapes, and a few restrained ${profile.icons}. Keep the child unmistakably central; any teacher, caregiver, or classmates must be simplified supporting characters, never additional focal children. Render this text clearly in the illustration: Chinese heading exactly "${concept.chinese}"; small English heading exactly "${concept.english}"; Chinese story sentence exactly "${story}"; small English story sentence exactly "${englishStory}". Put the two headings in a calm upper-corner hand-drawn label and the story sentences in a clean lower-third caption area. No other letters, numbers, logos, watermarks, school names, labels, or imitation artwork. Never use photorealism, camera lighting, realistic skin pores, 3D animation, busy rooms, deep perspective, clutter, distressing emotions, or unrelated props.`;
}
