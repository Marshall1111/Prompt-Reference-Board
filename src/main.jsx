...
function BodyBookPage() {
  ...
  // 重置提示词：工程态提交空值，由服务端恢复该页默认模板提示词；草稿态仅清除本地暂存编辑。
  async function resetPagePrompt(page) {
    if (!page || busyPageKey === page.key || ["queued", "running"].includes(page.status)) return;
    updatePagePrompt(page.key, "");
    if (!project) {
      dirtyPromptKeysRef.current = dirtyPromptKeysRef.current.filter((key) => key !== page.key);
      return;
    }
    await savePagePrompt(page, "");
    // 保存成功且期间未被再次编辑时，移除本地缓存以展示服务端返回的默认模板提示词。
    if (pagePromptsRef.current[page.key] === "") {
      pagePromptsRef.current = Object.fromEntries(Object.entries(pagePromptsRef.current).filter(([key]) => key !== page.key));
      setPagePrompts((current) => Object.fromEntries(Object.entries(current).filter(([key]) => key !== page.key)));
    }
  }
...
function BodyBookProjectItem({ page, onOpen, onDownload, onEditReferences, onGenerate, onDelete, busy, busyPageKey }) {
  ...
  const handleDelete = () => {
    void showAppDialog({ message: "确认删除本页？删除后本页卡片和图片将被移除。", mode: "confirm" }).then((confirmed) => {
      if (confirmed) onDelete();
    });
  };
  return <article className={`body-book-item body-book-project-item ${pending ? "is-pending" : ""} ${page.status === "failed" ? "is-failed" : ""}`}>
    {!page.isBuiltIn && !page.isRequired ? <button className="body-book-project-delete icon-button" disabled={busy || pending} onClick={handleDelete} title="删除页面" type="button"><X size={17} /></button> : null}
      ...
      {showCoinInfo ? (
        <div className="modal-backdrop draw-card-confirm" onClick={() => setShowCoinInfo(false)} role="presentation">
          <section className="draw-card-confirm-panel body-book-bean-modal" onClick={(event) => event.stopPropagation()} role="dialog" aria-modal="true" aria-label="我的币">
            ...
            {(visitorState?.redemptionEntitlements?.fridgeMagnetItemCount || 0) > 0 ? <p className="body-book-entitlement-note">实体冰箱贴兑换权益：<strong>{visitorState.redemptionEntitlements.fridgeMagnetItemCount}</strong> 枚</p> : null}
            ...
            <tbody>
              {users.map((user) => (
                <tr key={user.id}>
                  ...
                  <td className="user-admin-download">{user.recordType === "registered" ? `${Math.max(0, Number(user.redemptionEntitlements?.fridgeMagnetItemCount || 0))} 枚 / ${Math.max(0, Number(user.redemptionEntitlements?.bodyBookPrintCount || 0))} 册` : "—"}</td>
