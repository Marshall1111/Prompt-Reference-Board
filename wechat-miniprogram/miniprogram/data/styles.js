const styles = [
  ...
  },
  {
    ...
    "drawCardWeight": 100,
    "image": "/images-small/style_1787099592680.jpg",
    "prompt": "将上传Image1严格作为唯一视觉来源，制作竖版“原始摄影+抽象记忆面板”作品。画布上下各50%直接平接，无边框、阴影、相框、撕纸、胶带、悬浮卡片或拼贴。上半仅允许等比缩放及必要轻载，其余主体、人物、建筑、景物、光线、颜色、空间关系和摄影气质必须原样保留，禁止重画、扩图、修饰、替换、虚构、改形改位、风格化及任何滤镜。内部分析但不输出：识别3‑6个核心视觉事实，包括比例、节奏、明暗、颜色和留白；删除纹理、透视细节、背景杂讯及低信息装饰，仅留最低识别线索，按DECONSTRUCT→SELECTIVEPRESERVATION→ABSTRACT/DISTILL→RECONSTRUCT重构。下方背景固定为均匀无纹理暖象牙色#F3F0E8，以原图关系、轴线、间距、方向、层次和颜色构建克制稀疏母题，不复制物体轮廓，第一眼极简抽象，第二眼联想到原图；使用1个主要、最多2个辅助标记家族，母题占宽30%‑55%，至少65%留白，图形全部源自原图。建筑用垂直线、水平轴、斜线或层叠结构，最多留1‑3个身份特征；风景用色带、地貌、岸线和地平线；树木云朵采用重叠有机色块；人物化为不规则轻微收分短竖块；道路、水面、栏杆化为1‑2条细轴线；倒影上下呼应但不机械镜像。颜色仅取Image1并适度降饱和，限1主色、1深色结构色、1浅或中性色及最多1‑2个强调色。标题依据真实主体、光线、时间、方向或空间关系生成唯一2‑5词英文标题，简洁克制略有诗意，禁用Memory、Dream、Moment，不加解释、日期、地点、副标题；用优雅衬线字体置左下并留安全边距，颜色取原图深色调。禁止完整插画、第二风景画、图标、信息图、装饰对称、密集元素、渐变、纸纹、颗粒、噪点、发光、暗角、污渍、编号、色卡、图例、签名、Logo、水印及标题外文字。最终仅输出一张完成作品。",
    "useStyleImageAsReference": false
  },
  {
