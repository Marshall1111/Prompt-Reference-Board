const styles = [
  {
    ...
    "prompt": "把这 张照片改成画风笨拙又搞笑的涂鸦风插画 。 特点： - 营造出像是用平价马克笔、蜡笔随手画出来的潦草速写质感- 线条杂乱随性，充满孩子气画风- 透视扭曲失衡，整体构图比例不协调- 脸部五官稍作夸张处理，打造滑稽喜感 额外添加元素：- 简约漫画风背景（结合参考照片联想绘制即可）- 添上无意义的乱涂线条、细碎随性的小细节- 保留上色不均、涂色出界、明显的笔触痕迹 整体风格：- 透着敷衍潦草、毫无用心去画的随性感- 切勿画得太过精致完整- 沙雕搞笑的表情包趣味画风- 贴合网络轻松趣味的氛围 禁止要求：- 不要画得写实- 不要刻画得太过精致细腻- 不要做出专业规整的绘画效果。请根据上传图片的主体姿态、场景方向和视觉重心，自动选择最合适的画幅比例：横向构图使用 3:2，纵向构图使用 2:3。保持原图主要元素完整，不裁切主体，可自然扩展背景并微调元素位置，使画面更协调。"
  },
  {
  ...
  },
  {
    ...
    "prompt": "把参考图中的主体变成一个萌风Q版角色，凌乱粗糙的黑色线稿，幼态比例，像小孩随手涂鸦但极具设计感，粗笔触、水粉厚涂、蜡笔质感，边缘随意外扩，留白背景、可爱、手绘感极强，不精致，故意画歪，松散构图，低饱和奶油色调，艺术涂鸦插画。请根据上传图片的主体姿态、场景方向和视觉重心，自动选择最合适的画幅比例：横向构图使用 3:2，纵向构图使用 2:3。保持原图主要元素完整，不裁切主体，可自然扩展背景并微调元素位置，使画面更协调。"
  },
  {
    ...
    "prompt": "将图片风格化为皮克斯动画风格，角色造型圆润可爱，光影层次分明，保留人物或宠物原有姿态和互动关系，画面温暖明亮。请根据上传图片的主体姿态、场景方向和视觉重心，自动选择最合适的画幅比例：横向构图使用 3:2，纵向构图使用 2:3。保持原图主要元素完整，不裁切主体，可自然扩展背景并微调元素位置，使画面更协调。"
  },
  {
    ...
    "prompt": "请将整个图像转化为单一的装饰性民间平面插图，带有涂鸦元素。使用大胆而俏皮的色彩调色板，与原图像完全不同。\n将所有细节简化为干净、平面的形状，带有手工制作的、略显不完美感，仿佛绘制在一张白纸上。整体风格应显得可爱、童趣十足且异想天开。请根据上传图片的主体姿态、场景方向和视觉重心，自动选择最合适的画幅比例：横向构图使用 3:2，纵向构图使用 2:3。保持原图主要元素完整，不裁切主体，可自然扩展背景并微调元素位置，使画面更协调。"
  },
  {
    ...
    "prompt": "把图二中的猫咪换成图一中的狗狗，比例改成2:3."
  },
  {
  ...
  },
  {
    ...
    "prompt": "保留原图尺寸，在图片中的动物、食物和人物和有趣的元素周围用粗糙手绘描边，并点缀可爱的手写涂鸦和剪贴风格的注释（英文），还有俏皮的白色马克笔画、爱心、星星、蛋糕、笑脸和小型食物等图标，营造出一种可爱的Instagram美食氛围，8K画质。请根据上传图片的主体姿态、场景方向和视觉重心，自动选择最合适的画幅比例：横向构图使用 3:2，纵向构图使用 2:3。保持原图主要元素完整，不裁切主体，可自然扩展背景并微调元素位置，使画面更协调。"
  },
  {
  ...
  },
  {
    ...
    "prompt": "把照片生成いらすとや风格的图片。请根据上传图片的主体姿态、场景方向和视觉重心，自动选择最合适的画幅比例：横向构图使用 3:2，纵向构图使用 2:3。保持原图主要元素完整，不裁切主体，可自然扩展背景并微调元素位置，使画面更协调。"
  },
  {
    ...
    "prompt": "转绘成独立卡通插画，新天真流行艺术风格，丑萌趣味人物，超细黑色轮廓线，高饱和度明快平涂色彩，手绘随性不规则线条，低保真涂鸦质感，怪诞无厘头趣味，夸张面部神态，微猎奇却软萌可爱，极简背景，现代街头艺术氛围，摒弃精致流水线画风。请根据上传图片的主体姿态、场景方向和视觉重心，自动选择最合适的画幅比例：横向构图使用 3:2，纵向构图使用 2:3。保持原图主要元素完整，不裁切主体，可自然扩展背景并微调元素位置，使画面更协调。"
  },
  {
    ...
    "prompt": "给我生成一个照片，像灵魂画手在休息时间随便用中性笔勾勒几笔一样，线条简单利索，乍一看很随便甚至像用左手乱涂一样线条一抖一抖的，眼睛可以是豆豆眼也可以是一个横线条，反正不能展示一点画功，但该有的精髓都在，画面要带有莫名喜感和自信，有种越荒谬越搞笑越好，颜色要黑色线条，画面中只显示纸张和画就可以。请根据上传图片的主体姿态、场景方向和视觉重心，自动选择最合适的画幅比例：横向构图使用 3:2，纵向构图使用 2:3。保持原图主要元素完整，不裁切主体，可自然扩展背景并微调元素位置，使画面更协调。"
  },
  {
    ...
    "prompt": "请把附上的图片，用最粗糙、最寒碜的线条重新画一遍。背景是白色。要画出那种用鼠标在画图工具里画的、看似对又不太对、有点神韵但又很微妙地跑偏的感觉，直接把像素感拉满。总之请全力表现那种“寒碜感”。哎呀算了，随便画吧。请根据上传图片的主体姿态、场景方向和视觉重心，自动选择最合适的画幅比例：横向构图使用 3:2，纵向构图使用 2:3。保持原图主要元素完整，不裁切主体，可自然扩展背景并微调元素位置，使画面更协调。"
  },
  {
